...
class Graph {
  ...
  public boolean addEdge(Integer id1, Integer id2) {
    // YOUR WORK HERE
	  if(!storage.containsKey(id2))
		  return false;
	  List<Integer> value = storage.getOrDefault(id1, new ArrayList<Integer>());
	  value.add(id2);
	  storage.put(id1, value);
    return true;
  }
  ...
  public boolean removeEdge(Integer id1, Integer id2) {
    // YOUR WORK HERE
	  if(storage.containsKey(id1) && storage.containsKey(id2)) {
		  List<Integer> vertexes = storage.get(id1);
		  for(int i=0;i<vertexes.size();i++) {
			  if(vertexes.get(i) == id2) {
				  vertexes.remove(i);
				  return true;
			  }
